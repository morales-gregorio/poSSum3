...
from pos_parameters import filename_parameter, value_parameter, filename
...
class nonuniform_relice(generic_workflow):
    ...
    """
    _f = {
        'ref_input': filename('ref_input', work_dir='01_reference_input',
                              str_template='{idx:04d}.png'),
        'ref_mask': filename('ref_mask',  work_dir='02_reference_mask',
                             str_template='{idx:04d}.png'),
        'weighted_grayscale': filename('weighted_grayscale',
                                       work_dir='04_weighted_grayscale',
                                       str_template='{idx:04d}.nii.gz'),
        'weighted_multichannel': filename('weighted_multichannel',
                                          work_dir='05_weighted_multichannel',
                                          str_template='{idx:04d}.nii.gz'),
        'weighted_grayscale_mask': filename('weighted_grayscale_mask',
                                            work_dir='04_weighted_grayscale',
                                            str_template='????.nii.gz'),
        'weighted_multichannel_mask': filename('weighted_multichannel_mask',
                                               work_dir='05_weighted_multichannel',
                                               str_template='%04d.nii.gz'),
        'output_volumes': filename('output_volumes',
                                   work_dir='10_output_volumes',
                                   str_template='output_volume.nii.gz'),
        'tmp_gray_vol': filename('tmp_gray_vol',
                                 work_dir='09_intermediate_results',
                                 str_template='__temp__vol.vtk'),
        }
    _usage = ""
        ...
        for ref_coord in coords_to:
            try:
                ...
                slice_results = {'status': False, 'coord': i,
                                 'slice_1': self.CONST_NOSLICE_INDEX,
                                 'weight_1': 1.0,
                                 'slice_2': self.CONST_NOSLICE_INDEX,
                                 'weight_2': 1.0}
                results.append(slice_results)
    ...
    def _getCommandLineParser(cls):
        ...
        outputVolumeSettings.add_option(
            '--output-volume-spacing', default=[1, 1, 1],
        ...
        outputVolumeSettings.add_option(
            '--output-volume-permute-axes', default=[0, 1, 2],
        ...
        outputVolumeSettings.add_option(
            ...
            dest='output_volume_orientation', type='str',
            default='RAS', help='')
        outputVolumeSettings.add_option('--grayscale-volume-filename',
                                        dest='grayscaleVolumeFilename',
                                        type='str', default=None)
        outputVolumeSettings.add_option('--rgbVolumeFilename',
                                        dest='rgbVolumeFilename',
                                        type='str', default=None)
        outputVolumeSettings.add_option(
